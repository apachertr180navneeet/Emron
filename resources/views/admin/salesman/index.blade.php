...
@section('page_title', 'Salesman Master')
@section('content')
<div class="data-card">
        ...
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div class="d-flex align-items-center gap-2 gap-md-3 flex-grow-1 flex-md-grow-0">
                <div class="position-relative flex-grow-1" style="min-width:0;max-width:320px">
                    ...
                    <input type="text" id="searchInput" class="form-control ps-5" placeholder="Search salesman, email, mobile..." style="border-radius:.75rem;font-size:.8125rem;padding:.375rem .75rem" value="{{ request('search') }}">
                </div>
            ...
            <a href="{{ route('admin.salesman.create') }}" class="btn btn-success btn-sm px-3 flex-shrink-0" style="border-radius:.75rem;font-size:.75rem;font-weight:700">
    ...
    <div class="overflow-x-auto" id="tableContainer">
        @include('admin.salesman._table')
    </div>

    <div class="table-footer" id="paginationContainer">
        @include('admin.salesman._pagination')
    </div>
</div>
@endsection
@section('script')
...
function toggleStatus(el, id) {
    postForm('/admin/salesman/' + id + '/toggle-status', { _token: csrfToken }, function(res) {
        if (res.success) {
            ...
            Swal.fire({ title: 'Status Updated', text: 'Salesman status changed to ' + res.status, icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
        }
    });
}
...
function confirmDelete(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: 'This salesman will be deleted!',
        icon: 'warning',
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('searchBtn').addEventListener('click', function() {
        ...
        window.location.href = '{{ route("admin.salesman.index") }}?search=' + encodeURIComponent(search);
    });
