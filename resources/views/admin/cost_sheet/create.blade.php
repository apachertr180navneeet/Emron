...
@section('page_title', 'Add Cost Sheet')
@section('content')
<div class="data-card">
    <div class="d-flex align-items-center gap-3 px-4 py-3 border-bottom">
        ...
        <h5 class="card-section-title mb-0"><i class="bi bi-calculator"></i> Add Cost Sheet</h5>
    </div>
    <div class="p-4">
        <form action="{{ route('admin.cost-sheet.store') }}" method="POST" id="costSheetForm">
            @csrf
            ...
            <div class="row g-4 mb-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="bom_no" class="form-control @error('bom_no') is-invalid @enderror" value="{{ old('bom_no', $nextBomNo) }}" readonly>
                ...
                <div class="col-md-4">
                    ...
                    <select name="product_id" id="product_id" class="form-select @error('product_id') is-invalid @enderror" onchange="onProductChange()">
                        ...
                        <option value="{{ $item->id }}" data-name="{{ $item->item_name }}" {{ old('product_id') == $item->id ? 'selected' : '' }}>{{ $item->item_name }} ({{ $item->short_code }})</option>
                        @endforeach
                    </select>
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    ...
                    <input type="number" name="qty" id="qty" class="form-control @error('qty') is-invalid @enderror" placeholder="Enter qty" value="{{ old('qty') }}" min="0.01" step="0.01" oninput="onProductChange()">
                ...
                <div class="col-md-4 d-flex align-items-end">
                    <button type="button" class="btn btn-info btn-sm px-4" style="border-radius:.75rem;font-weight:600" onclick="loadBom()">
                        <i class="bi bi-search me-1"></i> Load BOM & Calculate
                    </button>
                </div>
            </div>
            ...
            <div id="bomSection" style="display:none">
                    ...
                    <table class="table table-bordered align-middle mb-0" style="font-size:.8125rem">
                        <thead>
                            <tr style="background:#f8fafc">
                                ...
                                <th class="px-3 py-2 text-center text-uppercase" style="font-size:.6875rem;font-weight:700;color:#94a3b8">BOM Qty</th>
                        ...
                        <tfoot id="bomFoot" style="display:none">
                            <tr>
                    ...
                    <table class="table table-bordered align-middle mb-0" style="font-size:.8125rem">
                        ...
                        <tbody id="expenseBody">
                            <tr class="expense-row">
                                ...
                                <td class="px-2 py-2">
                                    <input type="text" name="expenses[0][expense_name]" class="form-control form-control-sm expense-name" style="font-size:.8125rem" placeholder="Enter expense name" list="expenseList" oninput="calcCosts()">
                                ...
                                <td class="px-2 py-2">
                                    <input type="text" name="expenses[0][amount]" class="form-control form-control-sm text-center expense-amount" style="font-size:.8125rem" placeholder="0" value="" oninput="calcCosts()">
                                </td>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-3">
                        ...
                        <input type="text" id="rawMaterialCost" class="form-control text-end fw-bold" style="font-size:.875rem" value="0.00" readonly>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Factory Expense Cost</label>
                        <input type="text" id="expenseCost" class="form-control text-end fw-bold" style="font-size:.875rem" value="0.00" readonly>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Total Cost</label>
                        <input type="text" id="totalCost" class="form-control text-end fw-bold" style="font-size:.875rem" value="0.00" readonly>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Profit % <span class="text-danger">*</span></label>
                        <input type="number" name="profit_percent" id="profitPercent" class="form-control text-center @error('profit_percent') is-invalid @enderror" value="{{ old('profit_percent', '10') }}" min="0" step="0.01" oninput="calcCosts()">
                    ...
                    <div class="col-md-2 d-flex align-items-end justify-content-end">
                        <div class="bg-indigo-50 border border-indigo-200 rounded-3 p-3 text-center" style="background:#eef2ff;border-color:#c7d2fe;min-width:200px">
                            ...
                            <input type="text" id="sellingPrice" class="form-control text-center fw-bold text-indigo-700 border-0 bg-transparent" style="font-size:1.5rem;color:#4338ca" value="0.00" readonly>
                        </div>
                    </div>
                </div>
                ...
                <div class="pt-3 border-top d-flex gap-2 justify-content-end">
                    ...
                    <button type="button" class="btn btn-primary px-4" style="border-radius:.75rem;font-weight:600" id="finalSubmitBtn" onclick="submitForm('Final')" disabled>
...
function loadBom() {
    ...
    if (!productId) { Swal.fire({ title: 'Error', text: 'Please select a product.', icon: 'error', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 }); return; }
    if (!qty || qty <= 0) { Swal.fire({ title: 'Error', text: 'Please enter a valid quantity.', icon: 'error', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 }); return; }
    ...
    .then(res => {
        ...
        if (!res.success) {
            tbody.innerHTML = '<tr><td colspan="8" class="text-center py-3 text-secondary">' + res.message + '</td></tr>';
            document.getElementById('bomFoot').style.display = 'none';
            errDiv.style.display = 'none'; sucDiv.style.display = 'none'; finalBtn.disabled = true;
            rawMaterialItems = [];
            calcCosts();
            return;
        }
        ...
        document.getElementById('bomFoot').style.display = 'table-footer-group';
        ...
        if (res.has_errors) {
            ...
            sucDiv.innerHTML = '<i class="bi bi-check-circle me-1"></i> Sufficient stock available.';
    ...
    .catch(() => {
        Swal.fire({ title: 'Error', text: 'Failed to load BOM.', icon: 'error', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
    });
}
...
function renderBomTable() {
    ...
    rawMaterialItems.forEach((item, i) => {
        const stockStatus = item.sufficient
        ...
        tbody.innerHTML += `<tr data-index="${i}">
            ...
            <td class="px-3 text-center">${item.bom_qty || '-'}</td>
...
function checkStockAvailability() {
    ...
    rawMaterialItems.forEach(item => {
        if (!item.sufficient && item.available_stock !== undefined) {
            hasError = true;
            errors.push('Insufficient stock for ' + item.raw_material_name + '. Required: ' + item.required_qty + ', Available: ' + item.available_stock);
        }
    });
    if (hasError) {
        errDiv.style.display = 'block';
        errDiv.innerHTML = '<strong>Stock Shortages:</strong><br>' + errors.join('<br>');
        sucDiv.style.display = 'none';
        finalBtn.disabled = true;
    } else if (rawMaterialItems.length > 0) {
        errDiv.style.display = 'none';
        sucDiv.style.display = 'block';
        sucDiv.innerHTML = '<i class="bi bi-check-circle me-1"></i> Sufficient stock available.';
        finalBtn.disabled = false;
...
function addExpense() {
    ...
    tr.innerHTML = `
        ...
        <td class="px-2 py-2">
            <input type="text" name="expenses[${expenseIndex}][expense_name]" class="form-control form-control-sm expense-name" style="font-size:.8125rem" placeholder="Enter expense name" list="expenseList" oninput="calcCosts()">
        </td>
        <td class="px-2 py-2">
            <input type="text" name="expenses[${expenseIndex}][amount]" class="form-control form-control-sm text-center expense-amount" style="font-size:.8125rem" placeholder="0" value="" oninput="calcCosts()">
        </td>
        <td class="px-2 py-2 text-center">
            <button type="button" class="btn btn-link text-danger p-0 border-0" style="font-size:1rem" onclick="removeExpense(this)" title="Remove"><i class="bi bi-x-circle"></i></button>
        </td>
...
function submitForm(status) {
    const itemsPayload = rawMaterialItems.map(item => ({
        ...
        sufficient: item.sufficient !== undefined ? item.sufficient : true,
    }));
