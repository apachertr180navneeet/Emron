...
<div class="data-card">
    <div class="d-flex align-items-center gap-3 px-4 py-3 border-bottom">
        ...
        <h5 class="card-section-title mb-0"><i class="bi bi-pencil"></i> Edit Item</h5>
    </div>
    <div class="p-4">
        <form action="{{ route('admin.item.update', $item->id) }}" method="POST">
            ...
            <div class="row g-4">
                <div class="col-md-4">
                    ...
                    <input type="text" name="short_code" class="form-control @error('short_code') is-invalid @enderror" placeholder="e.g. ERLD" value="{{ old('short_code', $item->short_code) }}">
                    @error('short_code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-4">
                    <label class="form-label">Item Name <span class="text-danger">*</span></label>
                    <input type="text" name="item_name" class="form-control @error('item_name') is-invalid @enderror" placeholder="e.g. Emerald" value="{{ old('item_name', $item->item_name) }}">
                ...
                <div class="col-md-4">
                    ...
                    <select name="item_type" class="form-select @error('item_type') is-invalid @enderror">
                        ...
                        <option value="{{ $type }}" {{ old('item_type', $item->item_type) == $type ? 'selected' : '' }}>{{ $type }}</option>
                        @endforeach
                    </select>
            ...
            <div class="mt-4 pt-3 border-top d-flex gap-2 justify-content-end">
                ...
                <button type="submit" class="btn btn-primary px-4" style="border-radius:.75rem;font-weight:600">Update Item</button>
            </div>
        </form>
    </div>
</div>
