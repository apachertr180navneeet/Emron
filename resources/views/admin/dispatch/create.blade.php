...
@section('page_title', 'Add Dispatch')
@section('content')
<div class="data-card">
    <div class="d-flex align-items-center gap-3 px-4 py-3 border-bottom">
        ...
        <h5 class="card-section-title mb-0"><i class="bi bi-truck"></i> Add Dispatch</h5>
    </div>
    <div class="p-4">
        <form action="{{ route('admin.dispatch.store') }}" method="POST" id="dispatchForm">
            @csrf
            <h6 class="fw-bold text-dark mb-3" style="font-size:.8125rem">Dispatch Information</h6>
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    <label class="form-label">Date <span class="text-danger">*</span></label>
                    <input type="date" name="dispatch_date" class="form-control @error('dispatch_date') is-invalid @enderror" value="{{ old('dispatch_date', date('Y-m-d')) }}">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="challan_no" class="form-control @error('challan_no') is-invalid @enderror" placeholder="Auto or manual" value="{{ old('challan_no', $nextChallan) }}">
                ...
                <div class="col-md-4">
                    ...
                    <select name="dispatch_status" class="form-select @error('dispatch_status') is-invalid @enderror">
                        ...
                        <option value="Pending" {{ old('dispatch_status') == 'Pending' ? 'selected' : '' }}>Pending</option>
                        <option value="In Transit" {{ old('dispatch_status') == 'In Transit' ? 'selected' : '' }}>In Transit</option>
                        <option value="Delivered" {{ old('dispatch_status') == 'Delivered' ? 'selected' : '' }}>Delivered</option>
                        <option value="Cancelled" {{ old('dispatch_status') == 'Cancelled' ? 'selected' : '' }}>Cancelled</option>
                    </select>
            ...
            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    ...
                    <select name="customer_id" id="customer_id" class="form-select @error('customer_id') is-invalid @enderror" onchange="onCustomerChange(this)">
                        ...
                        <option value="{{ $customer->id }}" data-mobile="{{ $customer->mobile }}" {{ old('customer_id') == $customer->id ? 'selected' : '' }}>{{ $customer->customer_name }} ({{ $customer->firm_name ?? $customer->mobile }})</option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="customer_mobile" id="customer_mobile" class="form-control @error('customer_mobile') is-invalid @enderror" placeholder="Auto fetch" value="{{ old('customer_mobile') }}" readonly>
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="transport_name" class="form-control @error('transport_name') is-invalid @enderror" placeholder="Enter transport name" value="{{ old('transport_name') }}">
                        ...
                        <tr class="item-row">
                            ...
                            <td class="px-2 py-2">
                                <select name="items[0][lot_no]" class="form-select form-select-sm lot-select" style="font-size:.8125rem" onchange="onLotSelect(this)">
                                    ...
                                    <option value="{{ $item->short_code ?? $item->id }}" data-item-id="{{ $item->id }}" data-item-name="{{ $item->item_name }}">{{ $item->short_code }} - {{ $item->item_name }}</option>
                                    @endforeach
                                </select>
            ...
            <div class="pt-3 border-top d-flex gap-2 justify-content-end">
                ...
                <button type="submit" class="btn btn-primary px-4" style="border-radius:.75rem;font-weight:600">Save Dispatch</button>
            </div>
        </form>
    </div>
</div>
