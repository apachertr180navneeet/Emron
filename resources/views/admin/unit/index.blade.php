...
<div class="data-card">
        ...
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div class="d-flex align-items-center gap-2 gap-md-3 flex-grow-1 flex-md-grow-0">
                <div class="position-relative flex-grow-1" style="min-width:0;max-width:320px">
                    ...
                    <input type="text" id="searchInput" class="form-control ps-5" placeholder="Search unit..." style="border-radius:.75rem;font-size:.8125rem;padding:.375rem .75rem" value="{{ request('search') }}">
                </div>
            ...
            <a href="{{ route('admin.unit.create') }}" class="btn btn-success btn-sm px-3 flex-shrink-0" style="border-radius:.75rem;font-size:.75rem;font-weight:700">
    ...
    <div class="overflow-x-auto" id="tableContainer">
        @include('admin.unit._table')
    </div>

    <div class="table-footer" id="paginationContainer">
        @include('admin.unit._pagination')
    </div>
</div>
@endsection
@section('script')
...
function toggleStatus(el, id) {
    postForm(BASE_URL + '/admin/unit/' + id + '/toggle-status', { _token: csrfToken }, function(res) {
        if (res.success) {
            el.className = 'status-badge ' + res.status;
            el.textContent = res.status.charAt(0).toUpperCase() + res.status.slice(1);
            Swal.fire({ title: 'Status Updated', text: 'Unit status changed to ' + res.status, icon: 'success', toast: true, position: 'top-end', showConfirmButton: false, timer: 2000 });
        }
    });
}
...
function confirmDelete(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = BASE_URL + '/admin/unit/' + id;
...
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('searchBtn').addEventListener('click', function() {
        ...
        window.location.href = '{{ route("admin.unit.index") }}?search=' + encodeURIComponent(search);
    });
